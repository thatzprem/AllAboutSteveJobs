...
@synthesize picturesTitleArray = _picturesTitleArray;
...
- (void)setTitleWithCurrentPhotoIndex 
{
    ...
    NSString *title = [NSString stringWithString:[self.picturesTitleArray objectAtIndex:currentIndex_ + 1]];
    NSLog(@"%@",title);
   [self setTitle:title];
}
...
- (void)viewDidLoad 
{
   [super viewDidLoad];
    ...
    //Load the Picture individual names in an array
    NSString *headingPath = [[NSBundle mainBundle] pathForResource:@"PicturesTabTitles" ofType:@"txt"];
    NSString *headingString = [NSString stringWithContentsOfFile:headingPath encoding:NSUTF8StringEncoding error:NULL]; 
    self.picturesTitleArray = (NSMutableArray*)[headingString componentsSeparatedByString:@"|"];
